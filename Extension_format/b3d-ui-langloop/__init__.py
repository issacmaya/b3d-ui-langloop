...
class LANGSWITCH_OT_cycle_language(Operator):
    ...
    def execute(self, context):
        ...
        context.preferences.view.language = lang_list[next_idx]
        self.report({'INFO'}, f"{get_text('switch_to')} {lang_list[next_idx]}")
    ...
    def draw(self, context):
        ...
        for i in range(self.cycle_count):
            ...
            row.prop(
                self,
                f"language_{i}",
                text=f"{get_text('language', current_lang)} {i + 1}"
            )

        if self.cycle_count < 10:
            box.operator(
                "langswitch.add_language",
                text="Add Language",
                icon='ADD'
            )
